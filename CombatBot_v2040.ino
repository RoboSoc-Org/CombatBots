...
int ch1, ch2, ch3, ch4, ch1Map, ch2Map, ch3Map, ch4Map, HPWM, VPWM, WPWM, TestPWM,RWD, LWD, WPN;
...
void loop() {
  ...
  ch4 = pulseIn(RC_Ch1, HIGH, 30000); // Read the pulse width of
  ch3 = pulseIn(RC_Ch2, HIGH, 30000); // each channel
  ch2 = pulseIn(RC_Ch3, HIGH, 30000); // Read the pulse width of 
  ch1 = pulseIn(RC_Ch4, HIGH, 30000); // each channel
  ...
  WPWM = map(ch3,1000,2000,1250,2500);
  //WPWM = map(ch3, 1000, 2000, 0, 510);
  //TestPWM = map(ch4,1000,2000,0,510);
  
  //WPN = WPWM - (TestPWM - 255);
  LWD = VPWM - (HPWM - 255);  // Right Wheel Drive: 0-200 backwards, 200-300 Stationary, 300 - 510 forwards
  RWD = VPWM - (255 - HPWM); //   Left Wheel Drive: 0-200 backwards, 200-300 Stationary, 300 - 510 forwards
...
delayMicroseconds(WPWM);
digitalWrite(GPIO_1,LOW);
delayMicroseconds(20000-WPWM);
  //-------------------------------//
 //        Weapon
//--------------------------------//
/*
  if (WPN >= 300) {                         // Move Fowards
    analogWrite(Motor_3B, 0);
    analogWrite(Motor_3A, int(WPN - 255));
  } else if (WPN >= 200 && WPN <= 300) { // Stationary, apply brake
